...
\begin{frame}{Motivation}
	In medical imaging, radiography typically uses apertures to modulate the radiation emitted by an x-ray source and produce high-resolution images. \\ \smallskip \pause
	...
	\bigskip 
	\begin{figure}
		\includegraphics[width = .9\textwidth]{images/x_ray_schematic.png}
	\end{figure}
	}
\only<3->{
	\medskip
...
\begin{frame}{Modified Uniformly Redundant Array (MURA)}
	A MURA pattern $A$ consists of specified openings that has a corresponding decoding pattern $G$\footnote[1]{Gottesman and Fenimore (1989)}. \\ \pause
	\bigskip
	\only<2>{
		\begin{columns}
			 		\begin{column}{0.65\textwidth}
				 			Let $p$ be a prime number and $A = \{A_{ij}\}_{i,j=0}^{p-1}$ be the binary aperture array.  Set 
				 		\begin{equation*}
				 			A_{ij} = 
				 			\begin{cases}
				 				0 & \text{if } i =0 \\
	...
	\only<3>{
			\begin{columns}
			\begin{column}{0.65\textwidth}
				The decoding function $G$ is constructed as follows:
				\begin{equation*}
					G_{ij} = 
					\begin{cases}
						+1 & \text{if } i+j =0 \\
						+1 & \text{if } A_{ij} =1 , i+j  \neq 0 \\
						-1 & \text{if } A_{ij} =1 , i+j  \neq 0 
					\end{cases}
				\end{equation*}
			\end{column}
...
\begin{frame}{MURA aperture imaging}
	...
	\end{figure}
The observation $D$ is given by 
$$D = S * A + B$$
where $B$ is background noise. The MURA reconstruction is given by
$$\hat{S} = D * G$$
where $G$ is the decoding pattern.
\end{frame}
...
\begin{frame}{Experiments}
	\only<1->{\textbf{Experiment 1:} Classify original MNIST images\\}
	\only<1>{
	\vspace{3em}
	\begin{figure}
		\includegraphics[width = 0.8\textwidth]{images/classify_original.png}
	\end{figure}
	}
	\only<2->{\textbf{Experiment 2:} Classify encoded MNIST images\\}
	\only<2>{
	\vspace{3em}
	\begin{figure}
		\includegraphics[width = 0.8\textwidth]{images/classify_encoded.png}
	\end{figure}
	}
	\only<3->{\textbf{Experiment 3:} Reconstruct encoded MNIST images, then classify from reconstructions\\}
	\only<3>{
	\vspace{3em}
	\begin{figure}
